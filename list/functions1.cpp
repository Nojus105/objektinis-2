...
void Rusiuoti(list<Stud> &grupe, char rusiavimas, char gal, double &TotalTime)
{
    ...
    if (rusiavimas == 'v') // Sort by name
    {
        grupe.sort([](const Stud &a, const Stud &b) { return a.vard < b.vard; });
    }
    else if (rusiavimas == 'p') // Sort by surname
    {
        grupe.sort([](const Stud &a, const Stud &b) { return a.pav < b.pav; });
    }
    else if (rusiavimas == 'g') // Sort by grade
    {
        if (gal == '0') // Sort by average
        {
            grupe.sort([](const Stud &a, const Stud &b) { return a.vid < b.vid; });
        }
        else if (gal == '1') // Sort by median
        {
            grupe.sort([](const Stud &a, const Stud &b) { return a.med < b.med; });
        }
    }
